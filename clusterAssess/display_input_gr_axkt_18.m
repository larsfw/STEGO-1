...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% fighdl=display_input_gr_axkt_18(min_color, max_color, ny, nx, varargin)
%
% display a number of images in one window
% display_input_gr_axkt_18(min_color, max_color, ny, nx, color_map, images, image_titles)
...
%   color_map have the size n*3 otherwise it will be handled like an image
...
%           clicking key ''e''  or ''E'' export all windows as PNGs
...
%           clicking key ''w''  or ''W'' add the polygon masks to the workspace
%
...
%               please beware that varargin is a global variable and is not
%               identical with Cdata, but a global variable. 
%               Means that if you have replaced an image, you cannot access
%               it via E. -> Present it right before saving!
%               [E.g. you have a jet(128) image and press E -> A double
%               image will be saved!]
%Examples
% if 0
%     
% 	i1g = imread('cameraman.tif');
% 	i1 = repmat(i1g, [1 1 3]); i1(:, :, 2) = gammacorr(i1(:, :, 2), 0.5); i1(:, :, 3) = gammacorr(i1(:, :, 3), 1.5);
% 
% 	display_input_gr_axkt_18([], [], 1, 2, i1, i1g);
% 
%    
%     % now: zoom or set zoom off and click or press key 'p' or key 'c' or the arrnew_ow keys
%     % unzoom only available at the subwindow, where the zoom action is done before
% 
%     display_input_gr_axkt_18([], [], [], [], i1, i1g, 'cameraman', 'cameraman gray');  % with own titles (not required)
% 
%     % oder
%     camerapeople = {i1, i1g};
%     names = {'cameraman', 'cameraman gray'};
%     display_input_gr_axkt_18([], [], [], [], camerapeople{:}, names{:});  % with own titles (not required)
% 
%     % Manipulation of single windows:	
%     AA = get(gcf, 'children');
%     h_annot = annotation('textbox', [0.05, 0.05, 0.90, 0.05], 'String', 'Gelbes Kreuz', 'Interpreter', 'Latex', 'Fontsize', 12);
% 
%     lastOne = get(AA(1));
%     lastImage = lastOne.Children(end); %???
%     myImage = flipud(i1);
%     set(lastImage, 'cdata', myImage);  
%     plot(AA(2), 100, 100, 'y+');
%     S4 = 'Hello';
%     set(h_annot,'String',S4, 'Interpreter', 'none')
% end
% 
% Important subfunctions: Properties [Please be sure it is copied]
%			 newid [Has been BORROWED from a newer MATLAB software! Please treat with care!]
%
% keywords: @_labeling, @_relabeling, @_imageplot, @_imageprocessing
% 
%
% Hermann Gross (2011), Dimitri Bulatov (2014), Isabel Wayand (2016), Lukas Lucks (2018), Denis Debroize (2020)
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
switch nargin
    case {0, 1, 2, 3, 4}
        ...
        fprintf(1,' press ''e'' to export PNGs  \n ');
        fprintf(1,' press ''r'' to relabel  \n ');
        fprintf(1,' press ''a'' to approve  \n ');
        fprintf(1,' press ''s'' to save the mask  \n ');
fprintf(1,' press ''w'' to add the variable to the base workspace  \n ');
        return;
...
case {'e','E'}
                    ...
                    [FileName,PathName] = uiputfile('*.png','save fragment as');
                    axa  = axhdls(k);
                    xMinMax = round(get (axa, 'XLim'));
                    yMinMax = round(get (axa, 'YLim'));
                    % here the loop
                    for ii = 1:ni
                        C = varargin_i{ii};                        
                        C_cut = C(max(yMinMax(1), 1):min(yMinMax(2), size(C, 1)), max(xMinMax(1), 1):min(xMinMax(2), size(C, 2)), :);
                        name = [PathName FileName(1:end-4)];
                        imwrite(C_cut, [name num2str(ii) '.png']);
                    end
                case {'r','R'}
                    if(sum(size(mask) - size(varargin_i{1}(:,:,1))) ~= 0)
                        mask = zeros(size(varargin_i{1}(:,:,1)));
                ...
                case {'w','w'}
                    if(sum(size(mask) - size(varargin_i{1}(:,:,1))) == 0)
                        prompt = {'Enter name for mask'};
                        dlg_title = 'Input for position';
                        num_lines = 1;
                        answer = inputdlg(prompt,dlg_title,num_lines);
                        assignin('base', answer{:}, mask);
                        mask = [];
