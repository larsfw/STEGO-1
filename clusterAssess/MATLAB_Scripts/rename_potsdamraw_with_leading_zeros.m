...
folderPath = 'D:\waglar\PotsdamMATLAB\gt_input';
...
for i = 1:numel(files)
    ...
    formattedNumericParts{end+1} = sprintf('%02d', str2double(numericParts{end})); %#ok<SAGROW> 
    
    % Concatenate the parts into a single string
    formattedNumericString = strcat(formattedNumericParts{:});
