...
class DinoFeaturizerFusion1(nn.Module):
    ...
    def forward(self, img1, img2, n=1, return_class_feat=False):
        self.model.eval()
        with torch.no_grad():
            assert img1.shape[2] % self.patch_size == 0
            assert img1.shape[3] % self.patch_size == 0

            # get selected layer activations
            feat1, attn1, qkv1 = self.model.get_intermediate_feat(img1, n=n)
            feat1, attn1, qkv1 = feat1[0], attn1[0], qkv1[0]
            feat2, attn2, qkv2 = self.model.get_intermediate_feat(img2, n=n)
            feat2, attn2, qkv2 = feat2[0], attn2[0], qkv2[0]

            feat_h = img1.shape[2] // self.patch_size
            feat_w = img1.shape[3] // self.patch_size

            if self.feat_type == "feat":
                image_feat = (
                    feat1[:, 1:, :]
                    .reshape(feat1.shape[0], feat_h, feat_w, -1)
                    .permute(0, 3, 1, 2)
                ) + (
                    feat2[:, 1:, :]
                    .reshape(feat2.shape[0], feat_h, feat_w, -1)
                    .permute(0, 3, 1, 2)
                )
            elif self.feat_type == "KK":
                image_k = qkv1[1, :, :, 1:, :].reshape(
                    feat1.shape[0], 6, feat_h, feat_w, -1
                ) + qkv2[1, :, :, 1:, :].reshape(feat2.shape[0], 6, feat_h, feat_w, -1)
                B, H, I, J, D = image_k.shape
                image_feat = image_k.permute(0, 1, 4, 2, 3).reshape(B, H * D, I, J)
            else:
                raise ValueError("Unknown feat type:{}".format(self.feat_type))
            ...
            if return_class_feat:
                return (
                    feat1[:, :1, :]
                    .reshape(feat1.shape[0], 1, 1, -1)
                    .permute(0, 3, 1, 2)
                ) + (
                    feat2[:, :1, :]
                    .reshape(feat2.shape[0], 1, 1, -1)
                    .permute(0, 3, 1, 2)
                )
...
class DinoFeaturizerFusion2(nn.Module):
    ...
    def forward(self, img, ndsm, n=1, return_class_feat=False):
        self.model.eval()
        with torch.no_grad():
            assert img.shape[2] % self.patch_size == 0
            assert img.shape[3] % self.patch_size == 0

            # Get features from the DINO backbone for the image input
            feat, attn, qkv = self.model.get_intermediate_feat(img, n=n)
            feat, attn, qkv = feat[0], attn[0], qkv[0]
            ...
            ndsm_resized = F.interpolate(
                ndsm.to(torch.float32),
                size=(image_feat.shape[2], image_feat.shape[3]),
                mode="bilinear",
                align_corners=False,
            )
            img_with_ndsm = torch.cat((image_feat, ndsm_resized), dim=1)
        ...
        if self.proj_type is not None:
            code = self.cluster1(self.dropout(img_with_ndsm))
            if self.proj_type == "nonlinear":
                code += self.cluster2(self.dropout(img_with_ndsm))
        else:
            code = img_with_ndsm

        if self.cfg.dropout:
            return self.dropout(img_with_ndsm), code
        else:
            return img_with_ndsm, code


class DinoFeaturizerFusion3(nn.Module):
    ...
    def forward(self, img, ndsm, n=1, return_class_feat=False):
        ...
        # ndsm_norm = ndsm.to(torch.float32) / 255.0
        ndsm_resized = F.interpolate(
            ndsm.to(torch.float32),
            size=(image_feat.shape[2], image_feat.shape[3]),
            mode="bilinear",
        ...
        code_with_ndsm = torch.cat((code, ndsm_resized), dim=1)

        if self.cfg.dropout:
            return self.dropout(image_feat), code_with_ndsm
        else:
            return image_feat, code_with_ndsm
